...
coefficients can be much larger.  Classical algorithm analyses therefore
couple algebraic invariants with concrete arithmetic models
\citep{knuth1997seminumerical,brent2010modern}.  Formal developments need the
same separation.
...
The model reuses mathlib's inductive \code{ZNum}.  Its zero constructor is
...
reference model.  It does not assume the asymptotics of Lean's native
...
The positive kernel mirrors binary long division: it scans numerator bits,
updates a partial remainder, conditionally subtracts the divisor, and emits
one quotient bit.  The signed wrapper reproduces Lean's Euclidean \code{/}
...
The function \code{xgcdWithCost} follows the textbook Euclidean recurrence.
...
The code is built with Lean/mathlib 4.32.0 \citep{demoura2021lean4}.  The
...
steps.  Fast algorithms may also use modular reconstruction or lattice
reduction \citep{cohen1996hnf}.  The present model supplies a concrete cost for
