...
size~\citep{kannan1979polynomial}.  A mechanized treatment must expose more
...
and mathlib~\citep{demoura2021lean4,mathlib2020} and returns the same strong HNF and SNF
...
\begin{itemize}
  ...
  \item a faithful total implementation of the Smith Steps 4--7, including
...
For dimension \(n\), the trace length is at most \(n^3\).  The corresponding
...
recurrence~\citep{bird2011determinants}.  Every scalar addition and
...
pass again produces a proper common divisor.
...
levels within a uniform original-input envelope.
...
cost~\citep{brent2010modern}.  The matrix layer charges the actual scalar
...
assistants~\citep{divason2022snf}.  The distinguishing goal here is the
end-to-end connection between a faithful executable schedule, explicit
