...
public import NormalForms.Matrix.Hermite.Transform
...
private theorem algorithmFinRangeNodup : ∀ n, (List.finRange n).Nodup
  | 0 => List.nodup_nil
  | n + 1 => by
      ...
      · exact (algorithmFinRangeNodup n).map (Fin.succ_injective n)

@[implicit_reducible] private def algorithmFinFintype (n : Nat) : Fintype (Fin n) where
  elems := ⟨List.finRange n, algorithmFinRangeNodup n⟩
  complete := List.mem_finRange

attribute [local instance 20000] algorithmFinFintype
...
@[simp] theorem applyRowOperation_swap_succ_one_rowOne {m n : Nat} {R : Type _}
  ...
  | succ i =>
      ...
      have hne : (1 : Fin (m + 2)) ≠ i.succ.succ := by
        intro hEq
        rw [← Fin.succ_zero_eq_one] at hEq
        have hzero : (0 : Fin (m + 1)) = i.succ :=
          Fin.succ_injective (m + 1) hEq
        exact Fin.succ_ne_zero i hzero.symm
      simp only [applyRowOperation, hne, if_false, if_pos]
...
@[simp] theorem applyRowOperation_swap_succ_one_other {m n : Nat} {R : Type _}
  ...
  have hjeq1 : (j.succ : Fin (m + 2)) ≠ 1 := by
    ...
    exact hj0 (Fin.succ_injective (m + 1) h)
  have hjeqi : j.succ ≠ i.succ := by
    exact fun h => hji (Fin.succ_injective (m + 1) h)
  simp only [applyRowOperation, hjeqi, hjeq1, if_false]
  ...
  | succ n ih =>
      ...
      | succ m =>
          ...
          · intro i
            simp
          · have htail :
                tailCols (0 : _root_.Matrix (Fin (m + 1)) (Fin (n + 1)) R) =
                  (0 : _root_.Matrix (Fin (m + 1)) (Fin n) R) := by
                ext i j
                rfl
            rw [htail]
            exact ih (m := m + 1)
...
/-- Constructive first-nonzero search used by the compiled algorithm. -/
def firstNonzeroWithOps? {R : Type _}
...
@[simp] theorem firstNonzeroWithOps?_eq_firstNonzero?
...
def clearFirstColumnStep {m n : Nat} {R : Type _}
  ...
  if _hzero : ComputableEuclideanOps.isZeroB (t.B i.succ 0) = true then
    t
  else
    let tSwap := t.trans (LeftTransform.swap t.B i.succ (1 : Fin (m + 2)))
    let tBez := tSwap.trans (LeftTransform.topBezout tSwap.B)
    tBez.trans
      (LeftTransform.unitSmul tBez.B 0 (ComputableEuclideanOps.normUnitUnit (tBez.B 0 0)))
...
def topReductionCoeff {m n : Nat} {R : Type _}
  ...
  -(ComputableEuclideanOps.quot (A 0 j.succ) (A i.succ j.succ))
...
def reduceTopRowStep {m n : Nat} {R : Type _}
    ...
    {A : _root_.Matrix (Fin (m + 2)) (Fin (n + 1)) R}
    (i : Fin (m + 1)) (t : LeftTransform A) : LeftTransform A :=
  match hrow : firstNonzeroWithOps? (fun j : Fin n => t.B i.succ j.succ) with
  | none => t
  | some j =>
      t.trans <|
...
theorem applyRowOperation_reduceTop_pivot {m n : Nat} {R : Type _}
  ...
  simp [applyRowOperation, topReductionCoeff, ComputableEuclideanOps.quot_eq,
    EuclideanDomain.mod_eq_sub_mul_div, sub_eq_add_neg, mul_comm]
  ...
  · let tSwap := t.trans (LeftTransform.swap t.B i.succ (1 : Fin (m + 2)))
    ...
    have hdet :
      ...
      calc
        tBez.B 0 0 =
            (ComputableEuclideanOps.xgcd
              (tSwap.B 0 0) (tSwap.B 1 0)).gcd := by
    ...
    have hnormTop :
      ...
      calc
        ...
        _ = normalize ((ComputableEuclideanOps.xgcd
              (t.B 0 0) (t.B i.succ 0)).gcd) := by
          rw [hdet]
        _ = (ComputableEuclideanOps.xgcd
              (t.B 0 0) (t.B i.succ 0)).gcd :=
          (ComputableEuclideanOps.xgcd_gcd_normalized
            (t.B 0 0) (t.B i.succ 0)).symm
    have hgcdNe :
        (ComputableEuclideanOps.xgcd
          (t.B 0 0) (t.B i.succ 0)).gcd ≠ 0 := by
      intro hg
      exact htop
        ((ComputableEuclideanOps.xgcd_gcd_eq_zero_iff
          (t.B 0 0) (t.B i.succ 0)).mp hg).1
...
theorem clearFirstColumnStep_topLeft_eq_xgcd {m n : Nat} {R : Type _}
    [EuclideanDomain R] [NormalizationMonoid R] [ComputableEuclideanOps R] [DecidableEq R]
    {A : _root_.Matrix (Fin (m + 2)) (Fin (n + 1)) R}
      ...
      | succ j =>
          have hswapEq : tSwap.B j.succ.succ 0 = t.B j.succ.succ 0 := by
            simpa [tSwap, LeftTransform.swap, LeftTransform.trans] using
              congrFun
                (applyRowOperation_swap_succ_one_other t.B i j.succ
                  (Fin.succ_ne_zero j)
                  (ne_of_lt hj')) 0
    ...
    · have : j = i := Fin.ext hEq
      ...
      | succ i =>
          have hrowOneBefore : t.B 1 0 = 0 := by
            exact hprefix 0 (Nat.zero_lt_succ i.1)
...
theorem reduceTopRowLoop_lowerRight {m n : Nat} {R : Type _}
  ...
  | succ k ih =>
      ...
      | none =>
          unfold reduceTopRowStep
          rw [firstNonzeroWithOps?_eq_firstNonzero?, hrow]
...
theorem reduceTopRowLoop_reduced_prefix {m n : Nat} {R : Type _}
  ...
  | succ k ih =>
      ...
      · cases hcurr : firstNonzero? (fun s : Fin n => t'.B iCurr.succ s.succ) with
        | none =>
            rw [hloop, reduceTopRowStep, firstNonzeroWithOps?_eq_firstNonzero?, hcurr]
        ...
        | some p =>
            ...
            | some q =>
                have hrow' : firstNonzero? (fun s : Fin n => t.B i.succ s.succ) = some q := by
                  simpa [hRowEq] using hrow
                have hiCurr : i < iCurr := by
                  change i.1 < k
                  exact hi'
                have hsrcZero : t'.B iCurr.succ q.succ = 0 := by
                  exact hnf_rowBelow_zero_at_pivot hLower' hiCurr hrow
                have hprev :
...
/-- Constructive Hermite kernel; recursive state and helpers remain module-private. -/
public opaque hermiteNormalFormFin {m n : Nat} {R : Type _}
    [EuclideanDomain R] [NormalizationMonoid R] [ComputableEuclideanOps R]
    [DecidableEq R] [CanonicalMod R]
    (A : _root_.Matrix (Fin m) (Fin n) R) : HNFResultFin A := by
  induction n generalizing m with
  | zero =>
      refine
        { U := 1
  ...
  | succ n ih =>
      ...
      | succ m =>
          by_cases hcol :
              firstNonzeroWithOps? (fun i : Fin (m + 1) => A i 0) = none
          · let tailRes := ih (m := m + 1) (tailCols A)
            refine
              { U := tailRes.U
                U_cert := tailRes.U_cert
                H := tailRes.U * A
                equation := by rfl
                isHermite := ?_ }
            refine IsHermiteNormalFormFin.zeroCol _ ?_ ?_
            · exact firstCol_zero_mul tailRes.U A <|
                firstNonzero?_eq_none (fun i : Fin (m + 1) => A i 0) (by simpa using hcol)
            · simpa [tailCols_mul, tailRes.equation] using tailRes.isHermite
          · cases m with
            | zero =>
                have h00 : A 0 0 ≠ 0 := by
                  cases hfirst : firstNonzeroWithOps? (fun i : Fin 1 => A i 0) with
                  | none =>
                  ...
                  | some i =>
                      ...
                      | zero =>
                          simpa using
                            firstNonzero?_some_ne_zero
                              (fun i : Fin 1 => A i 0) (by simpa using hfirst)
                      | succ i =>
                          exact Fin.elim0 i
                let tNorm :=
                  LeftTransform.unitSmul A 0 (ComputableEuclideanOps.normUnitUnit (A 0 0))
            ...
            | succ m =>
                cases hfirst :
                    firstNonzeroWithOps? (fun i : Fin (m + 2) => A i 0) with
                | none =>
                    exact False.elim (hcol hfirst)
                | some i =>
                    ...
                    let tNorm :=
                      tSwap.trans <|
                        LeftTransform.unitSmul tSwap.B 0 <|
                          ComputableEuclideanOps.normUnitUnit (tSwap.B 0 0)
                    have hi0 : A i 0 ≠ 0 := by
                    ...
                    let tLift :=
                      LeftTransform.diagLift tClear.B lowerRes.U lowerRes.U_cert
                    ...
                    have hAfterLiftLower : lowerRight tAfterLift.B = lowerRes.H := by
                      simp [tAfterLift, tLift, LeftTransform.diagLift, LeftTransform.trans,
                        lowerRight_diagLiftMatrix_mul, lowerRes.equation]
                    ...
                    have hAfterLiftBelow : ∀ r : Fin (m + 1), tAfterLift.B r.succ 0 = 0 := by
                      intro r
                      simp [tAfterLift, tLift, LeftTransform.diagLift, LeftTransform.trans,
                        _root_.Matrix.mul_apply, diagLiftMatrix, hClearBelow,
