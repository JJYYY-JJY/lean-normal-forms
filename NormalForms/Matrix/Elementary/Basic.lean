...
theorem basisRow_smul_vecMul {m n R : Type _} [Fintype m] [DecidableEq m] [CommRing R]
  ...
  · intro x _ hx
    have hix : i ≠ x := by simpa [eq_comm] using hx
    simp [Pi.smul_apply, smul_eq_mul, hix]
  · intro hi
    exact (hi (Finset.mem_univ i)).elim
...
theorem mulVec_smul_basisCol {m n R : Type _} [Fintype n] [DecidableEq n] [CommRing R]
  ...
  · intro x _ hx
    simp [Pi.smul_apply, smul_eq_mul, hx]
  · intro hj
    exact (hj (Finset.mem_univ j)).elim
...
theorem rowOperationMatrix_mul {m n R : Type _} [Fintype m] [DecidableEq m] [CommRing R]
  ...
  | swap i j =>
      ...
      by_cases hr₁ : r = i
      · subst hr₁
        simp [rowOperationMatrix, applyRowOperation]
      · by_cases hr₂ : r = j
        · subst hr₂
          simp [rowOperationMatrix, applyRowOperation, hr₁]
...
theorem mul_columnOperationMatrix {m n R : Type _} [Fintype n] [DecidableEq n] [CommRing R]
  ...
  | swap i j =>
      ...
      by_cases hc₁ : col = i
      · subst hc₁
        simp [columnOperationMatrix, applyColumnOperation]
      · by_cases hc₂ : col = j
        · subst hc₂
          simp [columnOperationMatrix, applyColumnOperation, hc₁]
...
theorem bezoutReductionMatrix_mulVec {R : Type _} [EuclideanDomain R] [DecidableEq R]
    (a b : R) :
  ...
  · rw [bezoutReductionMatrix, if_neg hg]
    ext i
    fin_cases i
    · simpa [bezoutCoreMatrix, _root_.Matrix.mulVec, dotProduct, Fin.sum_univ_two, mul_comm] using
        (EuclideanDomain.gcd_eq_gcd_ab a b).symm
    · suffices h :
          -(b / EuclideanDomain.gcd a b) * a + (a / EuclideanDomain.gcd a b) * b = 0 by
        simpa [bezoutCoreMatrix, _root_.Matrix.mulVec, dotProduct, Fin.sum_univ_two] using h
      apply (mul_left_cancel₀ hg)
      rw [mul_zero]
      calc
        EuclideanDomain.gcd a b *
            (-(b / EuclideanDomain.gcd a b) * a + (a / EuclideanDomain.gcd a b) * b) =
          -(EuclideanDomain.gcd a b * (b / EuclideanDomain.gcd a b * a)) +
            EuclideanDomain.gcd a b * (a / EuclideanDomain.gcd a b * b) := by
              ring
        _ = -((EuclideanDomain.gcd a b * (b / EuclideanDomain.gcd a b)) * a) +
              (EuclideanDomain.gcd a b * (a / EuclideanDomain.gcd a b)) * b := by
              ring
        _ = -(b * a) + a * b := by
...
theorem det_bezoutReductionMatrix {R : Type _} [EuclideanDomain R] [DecidableEq R] (a b : R) :
  ...
  · suffices hdet :
        EuclideanDomain.gcdA a b * (a / EuclideanDomain.gcd a b) +
          EuclideanDomain.gcdB a b * (b / EuclideanDomain.gcd a b) = 1 by
      simpa [bezoutReductionMatrix, hg, bezoutCoreMatrix, _root_.Matrix.det_fin_two_of,
    ...
    calc
      EuclideanDomain.gcd a b *
        ...
        EuclideanDomain.gcdA a b *
          ...
          EuclideanDomain.gcdB a b *
            (EuclideanDomain.gcd a b * (b / EuclideanDomain.gcd a b)) := by
            ring
      ...
      _ = EuclideanDomain.gcd a b := by
            simp [EuclideanDomain.gcd_eq_gcd_ab, mul_comm]
